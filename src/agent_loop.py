...
- "Cookbook" = the LLM-serving subsystem (NOT chat sessions, NOT a recipe app). Routing:
  ...
  • Server offline but WoL-capable (list_cookbook_servers shows [offline, WoL available]) → call `wake_server` with the server name (and optionally `port` for the service, e.g. 11434 for Ollama) before proceeding with serve_model/download_model. The tool sends a magic packet and waits up to 90s for the host to come online.
